...
def plot_probabilities(path, grammar_path):
    ...
    for i, (nt, prods) in enumerate(prob_gram.items()):
        ...
        for j, prod in enumerate(prods):
            ...
            if line > 3:
                plt.plot(valores_x,valores_y, label=nome_da_linha, marker=lines[line], color=colors[c])
            else:
                plt.plot(valores_x,valores_y, label=nome_da_linha, linestyle=lines[line], color=colors[c])
            line += 1
            c += 1
        plt.legend()
        nnt = nt.replace("<","")
...
def generate_plots(problem, methods_df):
    ...
    colors = [(0,0,0), (0.5,0.5,0.5), (0.25,0.25,0.25), (0.375,0.375,0.375), (0.625,0.625,0.625)]
    # colors = ["#fd7f6f", "#7eb0d5", "#b2e061", "#bd7ebe", "#ffb55a", "#ffee65", "#beb9db", "#fdcce5", "#8bd3c7"]
    colors = [ "#7eb0d5", "#bd7ebe","#ffb55a",  "#beb9db","#ffee65", "#fdcce5", "#8bd3c7","#b2e061","#fd7f6f"]
    ...
    for df, path in methods_df:
        ...
        if line > 3:
            plt.plot(x, df['mean'], color=colors[c], marker = lines[line], label = path)
        else:
            plt.plot(x, df['mean'], color=colors[c], linestyle = lines[line], label = path)
        plt.fill_between(x, df['mean']-df['std'], df['mean']+df['std'], color=colors[c], alpha=0.3)
...
if __name__ == "__main__":
    ...
    plot_performance("srbench_1",
        "srbench/dataset1/depth5/5.0/0.5/",
        "srbench/dataset1/depth10/5.0/0.5/",
        "srbench/dataset1/depth15/5.0/0.5/",
    )


    # plot_performance("pagie_psge", 
    #     "sge3/sge/sge_stats_depth_limit/",
    #     "psge/sge/psge_stats_fixed/1.0/",
    #     "psge/sge/psge_stats_fixed_5/5.0/",
    # )
    # plot_performance("pagie_copsge", 
    #     "sge3/sge/sge_stats_depth_limit/",
    #     "co-psge/sge/copsge_stats/5.0/0.5/",
    #     "co-psge/sge/copsge_stats_10/10.0/0.5/",
    # )
